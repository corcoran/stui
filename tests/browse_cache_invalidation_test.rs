...
fn test_invalidate_empty_dir_clears_all_prefixes() {
    ...
    let root_cached = cache
        .get_browse_items(folder_id, None, folder_sequence)
        .expect("Failed to get root items");
    assert!(
        root_cached.is_some(),
        "Root cache should exist before invalidation"
    );

    let messages_cached = cache
        .get_browse_items(folder_id, Some("Messages/"), folder_sequence)
        .expect("Failed to get Messages/ items");
    assert!(
        messages_cached.is_some(),
        "Messages/ cache should exist before invalidation"
    );

    // Invalidate entire folder (empty dir_path)
    cache
        .invalidate_directory(folder_id, "")
    ...
    assert!(
        root_after.is_none(),
        "Root cache should be cleared after invalidate_directory with empty dir_path"
    );

    let messages_after = cache
        .get_browse_items(folder_id, Some("Messages/"), folder_sequence)
...
fn test_invalidate_specific_dir_only_clears_that_prefix() {
    ...
    let messages_items = vec![BrowseItem {
        name: "file1.txt".to_string(),
        item_type: "file".to_string(),
        size: 100,
        mod_time: "2025-01-01T00:00:00Z".to_string(),
    ...
    cache
        .invalidate_directory(folder_id, "Messages/")
    ...
    assert!(
        root_after.is_some(),
        "Root cache should NOT be cleared when invalidating Messages/"
    );
    ...
    assert!(
        messages_after.is_none(),
        "Messages/ cache should be cleared after invalidating Messages/"
    );
}
...
fn test_remote_file_deletion_clears_cache() {
    ...
    let messages_after_invalidation = cache
        .get_browse_items(folder_id, Some("Messages/"), new_sequence)
    ...
    assert_eq!(
        final_cached.len(),
        2,
        "Should have 2 items (test/ directory removed)"
    );
}
